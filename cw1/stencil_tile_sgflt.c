...
void stencil(const int nx, const int ny, const int width, const int height,
             double* /*restrict*/ image, double* /*restrict*/ tmp_image)
{
...
//we want to mirror the initialisation
//as this indicates how the data is structured
//in memory
//this should help with alignment
//
//#pragma simd vectorlength(4,8,16)
//#pragma vector always
  const int tile_size = 64;
  for (int jb = 0; jb <= ny+1; jb+=tile_size) {
    for (int ib = 0; ib <= nx+1; ib+=tile_size) {
...
#pragma vector always
	...
	(image[j+(i-1)*height]/10) + \
	(image[j+(i+1)*height]/10) + \
	(image[j-1+i*height] /10)  + \
	(image[j+1+i*height] /10);
        }
      }
    }
                ...
                double* image, double* tmp_image)
{
  ...
  for (int jb = 0; jb < ny; jb += tile_size) {
	...
	//so checkerboard is b&W
	//which is why we only do() 
	//on half the alloc'd
	//the other half being default
	//assume white ~blank
