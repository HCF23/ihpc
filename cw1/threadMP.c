...
// 2 threads for the single processor has 2 HW threads...serial still ;)
// #pragma omp for num_threads(2) schedule(static) collapse(2)
void stencil(const int nx, const int ny, const int width, const int height,
             double* image, double* tmp_image)
{
  for (int j = 1; j < ny + 1; ++j) {
#pragma omp for num_threads(2) schedule(static)
